...
SELECT users.first_name, users.last_name, posts.content, comments.body AS COMMENT_BODY
FROM comments 
...
SELECT posts.content, users.first_name AS post_author_first_name, users.last_name AS post_author_last_name, posts.title AS post_title, users.username AS comment_author_username, comments.body AS comment_body
FROM comments
INNER JOIN users ON (comments.user_id = users.id) 
INNER JOIN posts ON (comments.posts_id = posts.id) 
